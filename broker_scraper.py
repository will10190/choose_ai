...
def _make_mock_results(n: int = 8) -> list:
    mock_stocks = [("2330","台積電"),("2317","鴻海"),("2454","聯發科"),("3529","力旺"),("6274","台燿"),("8438","昶昕"),("3293","鈦象"),("4760","勤凱")]
    results = []
    for sid, name in mock_stocks[:n]:
        ...
        results.append({
            "代號": sid, "名稱": name, "收盤價": close, "今日漲跌 (%)": round(dpct, 2), "糾結度 (%)": round(random.uniform(1.5, 8.0), 2),
...
.side-by-side-header { text-align: center; padding-bottom: 10px; font-weight: bold; border-bottom: 2px solid #ddd; margin-bottom: 15px; }
</style>
...
with tab_screen:
    ...
    if start_btn:
        ...
        token = st.secrets.get("FINMIND_TOKEN", "").replace("\n","").replace("\r","").replace(" ","").strip()
        ...
        progress.progress(80); status.text(f"🎯 初步篩選到 {len(winner_sids)} 檔，載入外資資料...")
        inst_dict = load_foreign_data_for_winners(tuple(winner_sids), tuple(type_map.items()), token, c5_days=c5_days if use_c5 else 0) if winner_sids else {}
        ...
        for sid in winner_sids:
            ...
            results.append({
                "代號": sid, "名稱": name_map.get(sid, ""), "收盤價": latest_close, "今日漲跌 (%)": daily_pct, "糾結度 (%)": c2_res.get("tangle_spread_pct", 0.0),
                "乖離 20WMA (%)": round(bias_20w, 2), "外資連買 (天)": consecutive_days, "今日外資買超 (張)": int(table_foreign_buy),
                "大戶週變化 (人)": c34_res.get("whale_trend", 0), "散戶週變化 (人)": c34_res.get("total_trend", 0), "_price_df": price_df,
            })
    ...
    if st.session_state.scan_done:
        ...
        if not results: st.warning("😕 全市場掃描完畢，今日無完全符合條件之標的。")
            ...
            for hc, h in zip(hcols, ["代號","名稱（點擊看K線）","收盤價","今日漲跌%","糾結度%","乖離20WMA%","外資連買(天)","外資買超(張)","大戶週變化","散戶週變化"]):
                hc.markdown(f"<span class='result-header-col'>{h}</span>", unsafe_allow_html=True)
...
with tab_branch:
    ...
    with col_input:
        # 使用綁定 session_state 的 text_input
        ...
        if result_df is not None:
            if result_df.empty:
                ...
                with col_buy:
                    ...
                    st.dataframe(
                        df_buy[["股票", "買進張數", "賣出張數", "差額"]],
                        use_container_width=True, hide_index=True, height=dynamic_height
                    )

                with col_sell:
                    st.markdown("<div class='side-by-side-header' style='color: #26a69a;'>📉 賣超</div>", unsafe_allow_html=True)
                    st.dataframe(
                        df_sell[["股票", "買進張數", "賣出張數", "差額"]],
                        use_container_width=True, hide_index=True, height=dynamic_height
                    )
                ...
                st.download_button(
                    "⬇️ 下載完整 CSV",
                    data=csv_bytes,
                    file_name=f"{selected_branch['name']}_買賣超_{data_date.replace('/','')}.csv",
                    mime="text/csv",
                    key="dl_csv"
                )
